...
export function NotificationBell() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // Bell degrades silently if the engine is unreachable — not worth a page-level error.
    }
  }, []);
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="relative p-2 text-on-surface-variant hover:text-primary hover:bg-surface-container-low rounded-[10px] transition-colors"
        aria-label="Notifications"
      >
        <Icon name="notifications" className="text-[20px]" />
        {unreadCount > 0 && (
          <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-error rounded-full border-2 border-surface-bright" />
        )}
      </button>
      {open && (
